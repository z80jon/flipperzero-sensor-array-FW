...
GridEye gridEye_init(uint8_t addr, gridEyeFramerate frameRate) {
    ...
    ge.tempData = malloc(64, sizeof(uint16_t));

    //Release the i2c bus as the function calls will want to acquire it themselves
    furi_hal_i2c_release(I2C_BUS);

    //Make sure we're in normal mode
    furi_check(gridEye_wake(ge));
    uint8_t command[2] = {POWER_CONTROL_REGISTER, POWER__NORMAL_MODE};
    furi_hal_i2c_tx(I2C_BUS, addr, &command, 2, I2C_TIMEOUT);
...
gridEyeStatus gridEye_getStatus(GridEye* ge) {
    ...
    //TODO create criteria for judging status

    //TODO load it into the enum

    furi_hal_i2c_release(I2C_BUS);

    //TODO Return the num && update the struct
}
...
int gridEye_setFrameRate(GridEye* ge, enum gridEyeFramerate) {
    ...
    furi_check(
        gridEyeFramerate == GridEyeFrameRate_10FPS || gridEyeFramerate == GridEyeFrameRate_1FPS);
    ...
    furi_check(furi_hal_i2c_write_reg_8(
        I2C_BUS, ge->addr, FRAMERATE_REGISTER, gridEyeFramerate, I2C_TIMEOUT));

    //Update the data structure
    ge->frameRate = gridEyeFramerate;
...
int16_t gridEye_getTemperatureInt(GridEye ge, uint8_t pixelAddr) {
    if(ge.status != GridEyeStatus_OK) return 0xFFFF;
}
