    ...
    if(resolution == (uint8_t)VL53L5CX_RESOLUTION_4X4) {
        ...
        for(j = 0; j < (int8_t)4; j++) {
            for(i = 0; i < (int8_t)4; i++) {
                signal_grid[i + (4 * j)] = (signal_grid[(2 * i) + (16 * j) + (int8_t)0] +
                                            signal_grid[(2 * i) + (16 * j) + (int8_t)1] +
                                            signal_grid[(2 * i) + (16 * j) + (int8_t)8] +
                                            signal_grid[(2 * i) + (16 * j) + (int8_t)9]) /
                                           (uint32_t)4;
                range_grid[i + (4 * j)] =
                    (range_grid[(2 * i) + (16 * j)] + range_grid[(2 * i) + (16 * j) + 1] +
...
static uint8_t _vl53l5cx_send_xtalk_data(VL53L5CX_Configuration* p_dev, uint8_t resolution) {
    ...
    if(resolution == (uint8_t)VL53L5CX_RESOLUTION_4X4) {
        ...
        for(j = 0; j < (int8_t)4; j++) {
            for(i = 0; i < (int8_t)4; i++) {
                signal_grid[i + (4 * j)] =
                    (signal_grid[(2 * i) + (16 * j) + 0] + signal_grid[(2 * i) + (16 * j) + 1] +
...
uint8_t vl53l5cx_init(VL53L5CX_Configuration* p_dev) {
    ...
    status |= RdMulti(
        &(p_dev->platform), VL53L5CX_UI_CMD_START, p_dev->temp_buffer, VL53L5CX_NVM_DATA_SIZE);
    ...
    status |= vl53l5cx_dci_write_data(
        p_dev, (uint8_t*)&pipe_ctrl, VL53L5CX_DCI_PIPE_CONTROL, (uint16_t)sizeof(pipe_ctrl));
#if VL53L5CX_NB_TARGET_PER_ZONE != 1
    tmp = VL53L5CX_NB_TARGET_PER_ZONE;
    status |= vl53l5cx_dci_replace_data(
        p_dev, p_dev->temp_buffer, VL53L5CX_DCI_FW_NB_TARGET, 16, (uint8_t*)&tmp, 1, 0x0C);
#endif

    status |= vl53l5cx_dci_write_data(
        p_dev, (uint8_t*)&single_range, VL53L5CX_DCI_SINGLE_RANGE, (uint16_t)sizeof(single_range));

    tmp = (uint8_t)1;
    status |= vl53l5cx_dci_replace_data(
        p_dev, p_dev->temp_buffer, VL53L5CX_GLARE_FILTER, 40, (uint8_t*)&tmp, 1, 0x26);
    status |= vl53l5cx_dci_replace_data(
        p_dev, p_dev->temp_buffer, VL53L5CX_GLARE_FILTER, 40, (uint8_t*)&tmp, 1, 0x25);
...
uint8_t vl53l5cx_set_power_mode(VL53L5CX_Configuration* p_dev, uint8_t power_mode) {
    ...
    if(power_mode != current_power_mode) {
        ...
        case VL53L5CX_POWER_MODE_WAKEUP:
            ...
            status |= _vl53l5cx_poll_for_answer(p_dev, 1, 0, 0x06, 0x01, 1);
            break;

        case VL53L5CX_POWER_MODE_SLEEP:
            status |= WrByte(&(p_dev->platform), 0x7FFF, 0x00);
            status |= WrByte(&(p_dev->platform), 0x09, 0x02);
            status |= _vl53l5cx_poll_for_answer(p_dev, 1, 0, 0x06, 0x01, 0);
            break;
...
uint8_t vl53l5cx_start_ranging(VL53L5CX_Configuration* p_dev) {
    ...
    for(i = 0; i < (uint32_t)(sizeof(output) / sizeof(uint32_t)); i++) {
        if((output[i] == (uint8_t)0) || ((output_bh_enable[i / (uint32_t)32] &
        ...
        if(((uint8_t)bh_ptr->type >= (uint8_t)0x1) && ((uint8_t)bh_ptr->type < (uint8_t)0x0d)) {
            if((bh_ptr->idx >= (uint16_t)0x54d0) && (bh_ptr->idx < (uint16_t)(0x54d0 + 960))) {
                ...
                bh_ptr->size =
                    (uint16_t)((uint16_t)resolution * (uint16_t)VL53L5CX_NB_TARGET_PER_ZONE);
            }
    ...
    status |= vl53l5cx_dci_write_data(
        p_dev, (uint8_t*)&(output), VL53L5CX_DCI_OUTPUT_LIST, (uint16_t)sizeof(output));
    ...
    status |= WrMulti(
        &(p_dev->platform), VL53L5CX_UI_CMD_END - (uint16_t)(4 - 1), (uint8_t*)cmd, sizeof(cmd));
    status |= _vl53l5cx_poll_for_answer(p_dev, 4, 1, VL53L5CX_UI_CMD_STATUS, 0xff, 0x03);
...
uint8_t vl53l5cx_stop_ranging(VL53L5CX_Configuration* p_dev) {
    ...
    if((auto_stop_flag != (uint32_t)0x4FF) && (p_dev->is_auto_stop_enabled == (uint8_t)1)) {
        status |= WrByte(&(p_dev->platform), 0x7fff, 0x00);
...
uint8_t vl53l5cx_get_ranging_data(VL53L5CX_Configuration* p_dev, VL53L5CX_ResultsData* p_results) {
    ...
    for(i = (uint32_t)16; i < (uint32_t)p_dev->data_read_size; i += (uint32_t)4) {
        ...
        case VL53L5CX_AMBIENT_RATE_IDX:
            (void)memcpy(
                p_results->ambient_per_spad, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_NB_SPADS_ENABLED
        case VL53L5CX_SPAD_COUNT_IDX:
            (void)memcpy(
                p_results->nb_spads_enabled, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_NB_TARGET_DETECTED
        case VL53L5CX_NB_TARGET_DETECTED_IDX:
            (void)memcpy(
                p_results->nb_target_detected, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_SIGNAL_PER_SPAD
        case VL53L5CX_SIGNAL_RATE_IDX:
            (void)memcpy(
                p_results->signal_per_spad, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_RANGE_SIGMA_MM
        ...
        case VL53L5CX_MOTION_DETEC_IDX:
            (void)memcpy(
                &p_results->motion_indicator, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
        default:
            break;
...
#ifndef VL53L5CX_DISABLE_TARGET_STATUS
                p_results
                    ->target_status[((uint32_t)VL53L5CX_NB_TARGET_PER_ZONE * (uint32_t)i) + j] =
                    (uint8_t)255;
#endif
            }
        }
    ...
    footer_id = ((uint16_t)(p_dev->temp_buffer[p_dev->data_read_size - (uint32_t)4]) << 8) &
                0xFF00U;
...
uint8_t vl53l5cx_set_ranging_frequency_hz(VL53L5CX_Configuration* p_dev, uint8_t frequency_hz) {
    ...
    status |= vl53l5cx_dci_replace_data(
        p_dev, p_dev->temp_buffer, VL53L5CX_DCI_FREQ_HZ, 4, (uint8_t*)&frequency_hz, 1, 0x01);
...
uint8_t vl53l5cx_get_integration_time_ms(VL53L5CX_Configuration* p_dev, uint32_t* p_time_ms) {
    ...
    status |=
        vl53l5cx_dci_read_data(p_dev, (uint8_t*)p_dev->temp_buffer, VL53L5CX_DCI_INT_TIME, 20);
...
uint8_t
    ...
    if((integration < (uint32_t)2) || (integration > (uint32_t)1000)) {
        ...
        status |= vl53l5cx_dci_replace_data(
            p_dev, p_dev->temp_buffer, VL53L5CX_DCI_INT_TIME, 20, (uint8_t*)&integration, 4, 0x00);
    }
...
uint8_t vl53l5cx_set_sharpener_percent(VL53L5CX_Configuration* p_dev, uint8_t sharpener_percent) {
    ...
    if(sharpener_percent >= (uint8_t)100) {
        ...
        status |= vl53l5cx_dci_replace_data(
            p_dev, p_dev->temp_buffer, VL53L5CX_DCI_SHARPENER, 16, (uint8_t*)&sharpener, 1, 0xD);
    }
...
uint8_t vl53l5cx_get_target_order(VL53L5CX_Configuration* p_dev, uint8_t* p_target_order) {
    ...
    status |=
        vl53l5cx_dci_read_data(p_dev, (uint8_t*)p_dev->temp_buffer, VL53L5CX_DCI_TARGET_ORDER, 4);
...
uint8_t vl53l5cx_set_ranging_mode(VL53L5CX_Configuration* p_dev, uint8_t ranging_mode) {
    ...
    status |=
        vl53l5cx_dci_write_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_RANGING_MODE, (uint16_t)8);
...
uint8_t vl53l5cx_enable_internal_cp(VL53L5CX_Configuration* p_dev) {
    ...
    status |= vl53l5cx_dci_replace_data(
        p_dev,
        p_dev->temp_buffer,
        VL53L5CX_DCI_INTERNAL_CP,
        16,
        (uint8_t*)&vcsel_bootup_fsm,
        1,
        0x0A);
...
uint8_t vl53l5cx_dci_read_data(
    ...
    if((data_size + (uint16_t)12) > (uint16_t)VL53L5CX_TEMPORARY_BUFFER_SIZE) {
        ...
        status |=
            WrMulti(&(p_dev->platform), (VL53L5CX_UI_CMD_END - (uint16_t)11), cmd, sizeof(cmd));
        status |= _vl53l5cx_poll_for_answer(p_dev, 4, 1, VL53L5CX_UI_CMD_STATUS, 0xff, 0x03);
